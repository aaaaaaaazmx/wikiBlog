---
title: "详解Android Gradle生成字节码流程"
published: 2023-04-25
description: "详解Android Gradle生成字节码流程"
tags: ["字节码","JVM","专栏"]
category: "字节码专栏"
draft: false
slug: "bytecode-013"
...
### 2.1 Gradle基础
学习Gradle基础，可以参考[深入理解Android之Gradle](https://link.zhihu.com/?target=https%3A//blog.csdn.net/innost/article/details/48228651)和[【Android 修炼手册】Gradle 篇 -- Gradle 的基本使用](https://zhuanlan.zhihu.com/p/65249493)这两篇文章，重点掌握以下几点：

- Task是Gradle构建中最核心的概念，Android工程的构建过程也是被分成了无数个Task按照一定的顺序执行，最后输出apk产物；
- Gradle构建生命周期分三个阶段：**初始化阶段**，**配置阶段**和**执行阶段**，每个阶段都有不同的作用；
...
![image.png](https://cdn.nlark.com/yuque/0/2022/png/215777/1654796824806-8244fbfa-ca15-4459-bc1d-e3de09ffc813.png)
![image.png](https://cdn.nlark.com/yuque/0/2022/png/215777/1654796824695-b8ae4b8f-bd06-40fd-94ad-046468aa8806.png)
### 2.3 工程和调试
[StudyGradleDemo](https://link.zhihu.com/?target=https%3A//github.com/cloudmusic-android/StudyGradleDemo)是一个Demo工程，可以用于调试AGP和KGP编译过程，也可以用于阅读和分析AGP、KGP源码，读者可按需自行下载。

- AGP: 依赖包gradle-3.5.0-source.jar，入口类：AppPlugin
- KGP: 依赖包kotlin-gradle-plugin-1.3.50-source.jar，入口类：KotlinAndroidPluginWrapper

Gradle调试方法可以参照官方教程[Debugging build logic](https://link.zhihu.com/?target=https%3A//docs.gradle.org/current/userguide/troubleshooting.html%23sec%3Atroubleshooting_build_logic)。总结来说：

- 创建Remote调试任务；
- 命令行输入开始调试命令，注意红框中的任务也可以改成其他debug的任务：

![image.png](https://cdn.nlark.com/yuque/0/2022/png/215777/1654796824628-6be30197-c20a-4026-a972-6151f7742c51.png)
...
AndroidJavaCompile
### 3.3 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWagZEibUq2HAo9mV2AnsLcON82FXURBibhJRKJqhWzglVv1c7UicYWdaIQ/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWvxIFEa0AamTazQ1L9f9GGabhVicicbhI8eHDDKcza5Q40LDpiag1LUUjQ/640?wx_fmt=png&amp;from=appmsg)
上图中的判断条件是通过调试源码提炼出来的，从这些判断条件可以看出，开发过程中一些不经意的书写习惯可能会触发全量编译，所以我们应该有意识地改变这些书写习惯。另外Gradle官网也对一些判断条件作了解释，详情参阅[Incremental Java compilation](https://link.zhihu.com/?target=https%3A//docs.gradle.org/current/userguide/java_plugin.html%23sec%3Aincremental_compile)。
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWBUGYwWYSA6nnibmCiaDoZxibHiawLMHUv4GunzlDNO6zvHibaQJgK920YTg/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVW2CgNwssxnFmwY6c6A3hXkkUwmH5ALhH4JZwXvzIicod7JxgmicQIKDaw/640?wx_fmt=png&amp;from=appmsg)
### 四、Kotlin类文件字节码编译流程
### 4.1 任务名
...
### 4.3 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWV7EEcDIZb2Qh3TibSON6icgp6IQbN5H84ZM1s8ric3EGib5fGHJmv6eVtQ/640?wx_fmt=png&amp;from=appmsg)
...
kotlin.compiler.execution.strategy=???(可选项：daemon/in-process/out-of-process) org.gradle.daemon=???(可选项：true/false)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWrSUEWPWibSZmyHCtLPGEUN6ia7qEEic6njTGGbmETGrzOsoKKMqFlhr0A/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWd9D0MDvwXxZQczMDj06syRv4o8RVqeg9euS2FSQ1wrertT24iaNgfzw/640?wx_fmt=png&amp;from=appmsg)
...
- 将注解处理器修改为支持增量的注解处理器，修改方法请参考官网[Incremental annotation processing](https://link.zhihu.com/?target=https%3A//docs.gradle.org/nightly/userguide/java_plugin.html%23sec%3Aincremental_annotation_processing)；
- 如果类中有定义一些公有静态常量需要被外部引用，尝试改为静态方法去获取，而不是直接引用
### 5.2 类编译链过长

- 为了避免类的依赖链过长，我们应该尽可能拆分解耦业务，如推进组件化，并将模块之间的依赖关系改为二进制依赖而非源码依赖。只有这样才有可能减少类依赖链的长度，进而减少Task的执行时间。
### 六、总结
...
- [Incremental Java compilation](https://link.zhihu.com/?target=https%3A//docs.gradle.org/current/userguide/java_plugin.html%23sec%3Aincremental_compile)
- [Kotlin 实践及原理](https://link.zhihu.com/?target=http%3A//www.jackywang.tech/2019/01/30/Kotlin-%25E5%25AE%259E%25E8%25B7%25B5%25E5%258F%258A%25E5%258E%259F%25E7%2590%2586/)
- [Kotlin 编译之路 "Kotlin编译器"](https://zhuanlan.zhihu.com/p/76622754)
- [Incremental annotation processing](https://link.zhihu.com/?target=https%3A//docs.gradle.org/nightly/userguide/java_plugin.html%23sec%3Aincremental_annotation_processing)
