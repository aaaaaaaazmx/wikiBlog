---
title: "初中级Android开发社招面试(下)"
published: 2021-11-25
description: "初中级Android开发社招面试(下)"
tags: ["面试","面经"]
category: "面经"
draft: false
slug: "interview-010"
...
- 参考回答： 
   - 要测试Android应用程序，通常会创建以下类型自动单元测试 
      ...
      - [Android官网-建立本地单元测试](https://link.juejin.cn?target=https%3A%2F%2Fdeveloper.android.com%2Ftraining%2Ftesting%2Funit-testing%2Flocal-unit-tests.html)
      - **检测测试**：真机或模拟器上运行的单元测试，由于需要跑到设备上，比较慢，这些测试可以访问仪器（Android系统）信息，比如被测应用程序的上下文，一般地，依赖不太方便通过模拟框架模拟时采用这种方式；
      - [Android官网-建立仪表单元测试](https://link.juejin.cn?target=https%3A%2F%2Fdeveloper.android.com%2Ftraining%2Ftesting%2Funit-testing%2Finstrumented-unit-tests.html)
   - 注意：单元测试不适合测试复杂的UI交互事件
   - 推荐文章：[Android 单元测试只看这一篇就够了](https://juejin.cn/post/6844903645843030030)
...
- 参考回答： 
   ...
   - 因此，Java提供**ReferenceQueue**来处理引用对象的回收情况。当**SoftReference**所引用的对象被GC后，**JVM**会先将**softReference**对象添加到**ReferenceQueue**这个队列中。当我们调用**ReferenceQueue的poll()方法**，如果这个队列中不是空队列，那么将返回并移除前面添加的那个Reference对象。 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760640-ce1c4a18-7032-4227-93f5-7ffb4feae412.webp)
   - 推荐文章：[Java中的四种引用类型：强引用、软引用、弱引用和虚引用](https://link.juejin.cn?target=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000015282652%23articleHeader3)
#### 3、Apk的大小如何压缩 ？
...
- 参考回答： 
   - 一个完整APK包含以下目录（将APK文件拖到Android Studio）： 
      ...
      - **AndroidManifest.xml**：包含核心Android清单文件。该文件列出应用程序的名称，版本，访问权限和引用的库文件。该文件使用Android的二进制XML格式。 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760638-fc702a71-0e41-49e0-9e5d-a02dce0f03f3.webp)
   ...
   - **减少dex文件大小**
      - 添加资源混淆 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760648-ea23376a-97b7-46f8-aa36-a8d1cb12ddce.webp)
      ...
      - 推荐文章：[Android混淆最佳实践](https://link.juejin.cn?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2Fcba8ca7fc36d)
   - **减少lib文件大小**
      ...
      - 只需在build.gradle直接配置即可，NDK配置同理 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760624-8f61b82b-f67b-4d68-b995-6b90da15924f.webp)
   - 推荐文章： 
      - [APK瘦身](https://link.juejin.cn?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2F5921e9561f5f)
#### 4、如何通过Gradle配置多渠道包？

- 参考回答： 
   - 首先要了解设置多渠道的原因。在安装包中添加不同的标识，配合自动化埋点，应用在请求网络的时候携带渠道信息，方便后台做运营统计，比如说统计我们的应用在不同应用市场的下载量等信息
   - 这里以友盟统计为例 
      - 首先在manifest.xml文件中设置动态渠道变量： ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760649-3323f38b-cd67-45c4-9171-aa61dbbb6f02.webp)
      - 接着在app目录下的build.gradle中配置productFlavors，也就是配置打包的渠道： ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760983-71c62dfd-fdf2-4733-82a7-0b5ba9597a81.webp)
   ...
   - 推荐文章： 
      - [美团Android自动化之旅—Walle生成渠道包](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FMeituan-Dianping%2Fwalle)
#### 5、插件化原理分析
...
- 参考回答： 
   ...
   - 而**热修复**则是从修复bug的角度出发，强调的是在不需要二次安装应用的前提下修复已知的bug。能 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760994-bbd8f90a-e841-4086-a909-c5a57960b3ed.webp)
   - **类加载机制**
   ...
   - 推荐文章： 
      - [Android动态加载技术 简单易懂的介绍方式](https://link.juejin.cn?target=https%3A%2F%2Fsegmentfault.com%2Fa%2F1190000004062866%23articleHeader1)
      - [深入理解Android插件化技术](https://link.juejin.cn?target=https%3A%2F%2Fyq.aliyun.com%2Farticles%2F361233%3Futm_content%3Dm_40296)
      - [为什么要做热更新](https://link.juejin.cn?target=https%3A%2F%2Fwww.cnblogs.com%2Fbaiqiantao%2Fp%2F9160806.html)
#### 6、组件化原理
...
- 参考回答： 
   ...
   - 举个简单的模型例子 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760960-d29f4fd9-f933-4d6f-8700-8f00337f630a.webp)![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500760956-516d4372-a58f-4880-a1bc-958aa1f3a18d.webp) App是主application，ModuleA和ModuleB是两个业务模块（**相对独立，互不影响**），Library是基础模块，包含所有模块需要的依赖库，以及一些工具类：如网络访问、时间工具等
   ...
   - 推荐文章： 
      - [干货 | 从智行 Android 项目看组件化架构实践](https://link.juejin.cn?target=https%3A%2F%2Fmp.weixin.qq.com%2Fs%3F__biz%3DMjM5MDI3MjA5MQ%3D%3D%26mid%3D2697268363%26idx%3D1%26sn%3D3db2dce36a912936961c671dd1f71c78%26scene%3D21%23wechat_redirect)
#### 7、跨组件通信
...
- 常见的组件化方案如下 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761084-5efd994f-3887-4baa-94e9-a40df607aeeb.webp)
#### 8、组件化中路由、埋点的实现
...
- 参考回答： 
   ...
   - 推荐文章： 
      - [安卓组件化开源方案实现](https://juejin.cn/post/6844903565035569166)
#### 9、Hook以及插桩技术

- 参考回答： 
   - **Hook**是一种用于**改变API执行结果**的技术，能够将系统的API函数执行**重定向**（应用的**触发事件**和**后台逻辑处理**是根据事件流程一步步地向下执行。而**Hook**的意思，就是在事件传送到终点前截获并监控事件的传输，像个钩子钩上事件一样，并且能够在钩上事件时，处理一些自己特定的事件，例如逆向破解App） ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761532-c98e78d6-099b-41c9-bf31-3842a7c939bd.webp)
   ...
   - 推荐文章： 
      - [Android插件化原理解析——Hook机制之动态代理](https://link.juejin.cn?target=http%3A%2F%2Fweishu.me%2F2016%2F01%2F28%2Funderstand-plugin-framework-proxy-hook%2F)
      - [android 插桩基本概念](https://link.juejin.cn?target=https%3A%2F%2Fblog.csdn.net%2Ffei20121106%2Farticle%2Fdetails%2F51879047)
      - [Android逆向之旅](https://link.juejin.cn?target=http%3A%2F%2Fwww.520monkey.com%2F)
#### 10、Android的签名机制？
...
- 参考回答： 
   ...
   - 推荐文章： 
      - [一篇文章看明白 Android v1 & v2 签名机制](https://link.juejin.cn?target=https%3A%2F%2Fblog.csdn.net%2Ffreekiteyu%2Farticle%2Fdetails%2F84849651)
#### 11、v3签名key和v2还有v1有什么区别

- 参考回答： 
   - 在**v1版本**的签名中，签名以文件的形式存在于apk包中，这个版本的apk包就是一个标准的zip包，**V2**和**V1**的差别是**V2**是对整个zip包进行签名，而且在zip包中增加了一个**apk signature block**，里面保存签名信息。 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761426-d66bbe7b-ed52-4a02-8d3f-43b6ad8c68ef.webp)
   ...
   - **v3版本**签名块也分成同样的三部分，与v2不同的是在SignerData部分，v3新增了attr块，其中是由更小的level块组成。每个level块中可以存储一个证书信息。前一个level块证书验证下一个level证书，以此类推。最后一个level块的证书，要符合SignerData中本身的证书，即用来签名整个APK的公钥所属于的证书 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761465-5abaa81f-4b79-4460-b831-74117cf8cb19.webp)
   - 推荐文章： 
      - [APK 签名方案 v3](https://link.juejin.cn?target=https%3A%2F%2Fsource.android.google.cn%2Fsecurity%2Fapksigning%2Fv3)
      - [Android P v3签名新特性](https://link.juejin.cn?target=https%3A%2F%2Fxuanxuanblingbling.github.io%2Fctf%2Fandroid%2F2018%2F12%2F30%2Fsignature%2F)
...
- 参考回答： 
   ...
   - 推荐文章：[Android Developers 官方文档](https://link.juejin.cn?target=https%3A%2F%2Fdeveloper.android.com%2Fguide%2Ftopics%2Fmanifest%2Fuses-sdk-element.html%23ApiLevels)
#### 13、说下Measurepec这个类
...
- 参考回答： 
   ...
   - 决定因素：值由**子View的布局参数LayoutParams**和父容器的**MeasureSpec**值共同决定。具体规则见下图：![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761514-c903d9ea-b09f-46a9-95f3-43f28d5f079d.webp)
...
- 参考回答： 
   - Android中常用布局分为**传统布局**和**新型布局**
      ...
      - 新型布局（**可视化拖拽控件**、编写XML代码、代码生成）： 
         - **约束布局（ConstrainLayout）**： ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761508-6065144f-2d66-4e16-b8a3-291bc800984c.webp)
         - 注：图片出自Carson_Ho的[Android：常用布局介绍&属性设置大全](https://link.juejin.cn?target=https%3A%2F%2Fblog.csdn.net%2Fcarson_ho%2Farticle%2Fdetails%2F51719519)
...
- 参考回答： 
   ...
   - **动画播放顺序**：在Animator中，AnimatorSet正是通过playTogether()、playSequentially()、animSet.play().with()、before()、after()这些方法来控制多个动画协同工作，从而做到对动画播放顺序的精确控制 ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761773-1b68d137-e919-473a-b637-581d29cf1b33.webp)
...
- 参考回答： 
   ...
   - 推荐文章： 
      - [Glide 源码分析](https://link.juejin.cn?target=https%3A%2F%2Fp1-jj.byteimg.com%2Ftos-cn-i-t2oaga2asx%2Fgold-user-assets%2F2019%2F4%2F24%2F16a4ec49c3af1f5c~tplv-t2oaga2asx-image.image)
#### 17、如何绕过9.0限制？

- 参考回答： ![](https://cdn.nlark.com/yuque/0/2023/webp/215777/1698500761807-7c8a77f7-4de1-4d31-8c82-2a2d61e84ac9.webp)
...
- 参考回答： 
   ...
   - 关于kotlin的重要性，相信大家在日常开发可以体会到，应用到实际开发中，多采用语法糖（例如单列模式、空值判断、高阶函数等）
