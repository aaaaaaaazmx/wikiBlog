...
description: "Context到底是什么_"
tags: ["Android","基础"]
category: "android基础"
draft: false
slug: "android-basic-016"
---
...
而ContextImpl类则真正实现了Context中的所以函数，应用程序中所调用的各种Context类的方法，其实现均来自于该类。一句话总结：**Context的两个子类分工明确，其中ContextImpl是Context的具体实现类，ContextWrapper是Context的包装类**Activity，Application，Service虽都继承自ContextWrapper（Activity继承自ContextWrapper的子类ContextThemeWrapper），但它们初始化的过程中都会创建ContextImpl对象，由ContextImpl实现Context中的方法。


## Context的源头 ActivityThread
当应用进程通过 Zygote 孵化后，Context 的创建流程如下：

- 进程启动后：

系统调用 ActivityThread.main()（入口函数），初始化主线程的 Looper 和 Handler。

- 创建 ContextImpl：

ActivityThread 通过 createBaseContext() 创建 ContextImpl（Context 的具体实现类）。

这是 Context 的真正源头，包含了应用的核心功能（如资源访问、启动组件等）。

- 绑定到组件：

Activity、Service 等组件的 Context 本质是 ContextImpl 的包装（如 Activity 的 Context 通过 attach() 方法绑定）。

**关键类与流程**
- ContextImpl：

Context 的具体实现类，提供实际功能（如 getResources()、startActivity()）。

- ActivityThread：

应用的主线程管理者，负责创建 ContextImpl 并绑定到组件。

- ContextWrapper：

Activity、Service 等组件的 Context 是它的子类，内部委托给 ContextImpl。

> Zygote 的间接作用：

Zygote 孵化了应用进程，但 Context 的初始化由 ActivityThread 在进程内完成。

Context 的直接源头：

是 ActivityThread 创建的 ContextImpl 对象，而非 Zygote。
