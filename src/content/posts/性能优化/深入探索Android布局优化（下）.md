...
description: "深入探索Android布局优化（下）"
tags: ["性能优化","Android"]
category: "性能优化"
draft: false
slug: "performance-017"
---

[https://juejin.cn/post/6844904048068395015](https://juejin.cn/post/6844904048068395015)

在上篇文章中，笔者带领大家学习了布局优化涉及到的绘制原理、优化工具、监测手段等等知识。如果对这块内容还不了解的建议先看看[《深入探索Android布局优化（上）》](https://juejin.im/post/6844904047355363341)、[深入探索Android布局优化（中）](https://juejin.im/post/6870445845834104846)。本篇，为深入探索Android布局优化的下篇。这篇文章包含的主要内容如下所示：
...
### 六、布局优化常规方案
布局优化的方法有很多，大部分主流的方案笔者已经在[Android性能优化之绘制优化](https://link.juejin.cn?target=https%3A%2F%2Fjsonchao.github.io%2F2019%2F07%2F28%2FAndroid%25E6%2580%25A7%25E8%2583%25BD%25E4%25BC%2598%25E5%258C%2596%25E4%25B9%258B%25E7%25BB%2598%25E5%2588%25B6%25E4%25BC%2598%25E5%258C%2596%2F)里讲解过了。下面，我将介绍一些其它的优化方案。
#### 1、布局Inflate优化方案演进
##### 1、代码动态创建View
...
在使用子线程创建视图控件的时候，我们可以把子线程Looper的MessageQueue替换成主线程的MessageQueue，在创建完需要的视图控件后记得将子线程Looper中的MessageQueue恢复为原来的。在[Awesome-WanAndroid](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FJsonChao%2FAwesome-WanAndroid)项目下的UiUtils的Ui优化工具类中，提供了相应的实现，代码如下所示：
```
java
 /**
...
由于AsyncLayoutInflater仅仅只能通过侧面缓解的方式去缓解布局加载的卡顿，因此，我们下面将介绍一种从根本上解决问题的方案。对于AsynclayoutInflater的改进措施，可以查看祁同伟同学封装之后的代码，具体的改进分析可以查看[Android AsyncLayoutInflater 限制及改进](https://link.juejin.cn?target=https%3A%2F%2Fwww.jianshu.com%2Fp%2Ff0c0eda06ae4)，这里附上改进之后的代码：
```
ini
/**
* 实现异步加载布局的功能，修改点：
...
答案就是X2C。
#### X2C
[X2C项目地址](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FiReaderAndroid%2FX2C%2Fblob%2Fmaster%2FREADME_CN.md)
...
首先，对于Android视图绘制的原理，我们必须要有一定的了解，关于这块，大家可以参考下[Android View的绘制流程](https://link.juejin.cn?target=https%3A%2F%2Fjsonchao.github.io%2F2018%2F10%2F28%2FAndroid%2520View%25E7%259A%2584%25E7%25BB%2598%25E5%2588%25B6%25E6%25B5%2581%25E7%25A8%258B%2F)这篇文章。
对于视图绘制的性能瓶颈，大概有以下三点：
...
#### 3、过渡绘制优化
在视图的绘制优化中，还有一个比较重要的优化点，就是避免过渡绘制，这个笔者已经在[Android性能优化之绘制优化](https://link.juejin.cn?target=https%3A%2F%2Fjsonchao.github.io%2F2019%2F07%2F28%2FAndroid%25E6%2580%25A7%25E8%2583%25BD%25E4%25BC%2598%25E5%258C%2596%25E4%25B9%258B%25E7%25BB%2598%25E5%2588%25B6%25E4%25BC%2598%25E5%258C%2596%2F)一文的第四小节详细分析过了。最后这里补充一下自定义View中使用clipRect的一个实例。
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757484766-6545758b-ee45-46ea-96b0-2da0013a06c1.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757507613-0e830d1e-c8ac-4294-9e23-b0af12e67be2.png)
**注意：**
...
- 3、TextView的优化，比如利用它的drawableLeft属性。此外，也可以**使用Android 9.0之后的 PrecomputedText，它将文件的measure与layout过程进行了异步化**。但是需要注意，如果要显示的文本比较少，反而会造成不必要的Scheduling delay，**建议文本字符大于200时才使用，并记得使用其兼容类PrecomputedTextCompat**，它在9.0以上使用PrecomputedText进行优化，在5.0~9.0使用StaticLayout进行优化。具体调用代码如下所示：Future future = PrecomputedTextCompat.getTextFuture( “text”, textView.getTextMetricsParamsCompat(), null)； textView.setTextFuture(future);
...
```
显示效果如下所示：
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757561617-b63edc8e-2efb-4678-be8d-630103c9cbe4.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757610206-3e9d3f84-d2d1-4e64-8984-7fc6d4876741.png)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWcsHyhrM77sPT7RoV70GHNic42HXnRdyIQ1BZ0Wy4KRCiapk3gyqSQHGQ/640?wx_fmt=png&amp;from=appmsg)
...
- ![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757632458-8c6b0578-c1c6-44dc-ba72-f4f89637ccdb.png)
##### 2、布局自动扁平化
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757641015-75b48ce3-4c33-4264-b6bb-845f3f0c815b.png)
##### 3、更细粒度地优化RecyclerView中组件的缓存与回收的方法
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWMiaFxadcRsWsKPsiaYckQKEcfjiaR6jFNmUW4ZUW7pkK0IgIoKGloBgnw/640?wx_fmt=png&amp;from=appmsg)
...
##### 小结
使用Litho，在布局性能上有很大的提升，但是开发成本太高，因为需要自己去实现很多的组件，并且其组件需要在编译时才能生成，不能够进行实时预览，但是可以把Litho封装成Flexbox布局的底层渲染引擎，以此实现上层的动态化，具体实现原理可参见[Litho在美团动态化方案MTFlexbox中的实践](https://link.juejin.cn?target=https%3A%2F%2Ftech.meituan.com%2F2019%2F09%2F19%2Flitho-practice-in-dynamic-program-mtflexbox.html)。
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWz9ka2icD4f8atHouCGKJYtPvYtejkPGicHsHxzL1zJWwdI7xSKadPiaAA/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757697322-24ade6d8-4594-400a-839a-d032978c6706.png)
...
![image.png](https://cdn.nlark.com/yuque/0/2023/png/215777/1698757708147-51110548-5483-478c-a6fe-0522cbfc0a0f.png)
绘制步骤大致如下：
...
由于RenderScript主要是用于音视频、图像处理等细分领域，这里笔者就不继续深入扩展了，对于NDK、音视频领域的知识，笔者在今年会有一系列学习计划，目前大纲已经定制好了，如果有兴趣的朋友，可以了解下：[Awesome-Android-NDK](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FJsonChao%2FAwesome-Android-NDK)。
### 八、布局优化的常见问题
