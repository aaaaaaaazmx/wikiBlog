...
description: "1 、应用启动的类型"
tags: ["性能优化","Android"]
category: "性能优化"
draft: false
slug: "performance-002"
---

[https://juejin.cn/post/6844904093786308622](https://juejin.cn/post/6844904093786308622)
...
具体的实例代码可在 [这里查看](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2FJsonChao%2FAwesome-WanAndroid%2Fblob%2Fmaster%2Fapp%2Fsrc%2Fmain%2Fjava%2Fjson%2Fchao%2Fcom%2Fwanandroid%2Fui%2Fmainpager%2Fadapter%2FArticleListAdapter.java)。
...
借鉴Facebook的 [profilo](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Ffacebookincubator%2Fprofilo) 工具原理，对启动整个流程进行耗时监控，在后台对不同的版本做自动化对比，监控新版本是否有新增耗时的函数。
# 四、启动优化常规方案
### 启动过程中的常见问题
...
#### 实战
这里我们直接使用维数的 [epic](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Ftiann%2Fepic) 对Thread进行Hook。在attachBaseContext中调用DexposedBridge.hookAllConstructors方法即可，如下所示：
```
java
...
- 4、异步启动器
### 4、异步优化最优解：异步启动器
[异步启动器源码及使用demo地址](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Fzeshaoaaa%2FLaunchStarter)
#### 常规异步优化痛点
...
#### 启动器流程
启动器的流程图如下所示：
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWwIBsmXiaptzWUdvmVKpKkOfbxCHPeTOCibgylmibdlwpZlG8BCkD5P3Dg/640?wx_fmt=png&amp;from=appmsg)
...
### 3、延迟优化最优解：延迟启动器
[延迟启动器源码及使用demo地址](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Fzeshaoaaa%2FLaunchStarter)
#### 核心思想
...
[Multidex优化Demo地址](https://link.juejin.cn?target=https%3A%2F%2Fgithub.com%2Flanshifu%2FMultiDexTest)
#### 注意
...
- 2、一般程序的压栈出栈操作也需要一定的代码，这段代码完成栈指针调整、参数传递、现场保护和恢复等操作。 若函数的函数体代码量小于编译器生成的函数压栈出栈代码，则可以放心地定义为inline，这个时候占用内存空间反而会减小。而当函数体代码大于函数压栈出栈代码时，将函数定义为inline就会增加内存空间的使用。
...
如有兴趣的同学可以看看这篇文章：[抖音BoostMultiDex优化实践：Android低版本上APP首次启动时间减少80%](https://juejin.im/post/6844904079206907911)
## 7、类预加载优化
