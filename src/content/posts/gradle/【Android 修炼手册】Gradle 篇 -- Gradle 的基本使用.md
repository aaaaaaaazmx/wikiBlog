---
title: "【Android 修炼手册】Gradle 篇 -- Gradle 的基本使用"
published: 2020-05-18
description: "【Android 修炼手册】Gradle 篇 -- Gradle 的基本使用"
tags: ["Gradle","构建工具"]
category: "gradle"
draft: false
slug: "gradle-005"
...
本文将从下面几个部分进行讲解：
![](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWpRvxm6gPx57b1ibfBlDTHpXdRdOqGeYSWvCbTujiaibf7hO2Ylgdiawajg/640?wx_fmt=png&amp;from=appmsg)
### 一、gradle 是什么
![](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWfmBm9x93mGK07voU2BIQptsy4sC3QACicJzuxxBsFQGicIbusXfrzOag/640?wx_fmt=png&amp;from=appmsg)官方解释是: Gradle is an open-source build automation tool focused on flexibility and performance. Gradle build scripts are written using a Groovy or Kotlin DSL.
可以从三个角度来理解
**1. gradle 是一个自动化构建工具**
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579577446274-05aca5d2-e42a-4452-9e35-63ab41678eb1.png)
...
### 二、gradle 项目分析
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWp0LFWibzo52W4JQibt1FZQicAQPwddq8RUfaFibAMxaGJnwmJBeibhSwQRA/640?wx_fmt=png&amp;from=appmsg)
关于 gradle 的项目层次，我们新建一个项目看一下，项目地址在  [EasyGradle](https://github.com/5A59/android-training/tree/master/gradle/EasyGradle)
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579587574696-5e9eee1a-3d28-40a2-a1f2-fc85e0778b47.png)
#### 2.1 settings.gradle
...
其依赖关系如下：
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579593625379-d4fafef9-c8ee-4c1a-8def-ed00653eccf7.png)
**implementation 依赖**
当 module1 使用 implementation 依赖 module2 时，在 app 模块中无法引用到 Module2Api 类
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579593654010-2adf857b-79d3-40cc-bda2-103241adc63d.png)
**api 依赖**
当 module1 使用 api 依赖 module2 时，在 app 模块中可以正常引用到 Module2Api 类，如下图
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579593684092-1da308c4-c9e1-482e-9b5e-66b80eb35e99.png)
**compileOnly 依赖**
当 module1 使用 compileOnly 依赖 module2 时，在编译阶段 app 模块无法引用到 Module2Api 类，module1 中正常引用，但是在运行时会报错
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579593984926-f4d39fec-2829-485c-8ff7-0eb9ce5bd500.png)
反编译打包好的 apk，可以看到 Module2Api 是没有被打包到 apk 里的
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579594014358-ab9ed7d6-a7d1-4280-be4b-64bfa32d83d9.png)
**runtimeOnly 依赖**
当 module1 使用 runtimeOnly 依赖 module2 时，在编译阶段，module1 也无法引用到 Module2Api
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579594127517-4c370bfe-1d3d-41bb-8499-35b0ca2984f6.png)
#### 2.5 flavor
...
### 三、gradle wrapper
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWgKTnrWygsOC6ljNjFgpQXXUAdhMpdpKGS9OLzvKibGiaYciaHS7d9gcsw/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWZ4iciaZj4d8t17b0ghW0koAnwrdUvt5iaVpygK8iaO4DStDRhyiboMQrE0A/640?wx_fmt=png&amp;from=appmsg)
...
### 五、gradle 生命周期及回调
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWCribhP0YMf4Fp1MhA9iaLvpaZVJvia9NDtnqedL6p4m8InICVRUvpXS0A/640?wx_fmt=png&amp;from=appmsg)
gradle 构建分为三个阶段
**初始化阶段**
...
```
### 六、自定义 task
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWn3LJeRh2aJI5iastgicS0K1skvhTcnXSOpDKnrRNib0F5GIKVibRQ0cVvA/640?wx_fmt=png&amp;from=appmsg)
...
```
### 七、Android transform
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWuic7oElxGF7dOJbNQYJicvt7RW27vt8uxsiaH1nice5pia7JkaEicbWk6K5A/640?wx_fmt=png&amp;from=appmsg)
...
### 八、自己写 plugin
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWYX6ERDjLWfk97YBUsNT34L39mg6ubSchPicJrttKRyBVnIf1iaZ7QeMA/640?wx_fmt=png&amp;from=appmsg)
...
```
现在为止，项目结构是这个样子的
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579596783683-1999b688-cf50-4296-a7b6-002d0084c3af.png)
#### 8.2 创建 Plugin
...
```
然后运行
./gradlew tasks --all | grep mytask，就可以看到我们在 plugin 里新增的 task 了
...
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579596932324-44ec153d-f876-4475-883d-d979bf99ca80.png)

![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579596942523-d13c8555-ebbe-475f-b22a-16738e6be422.png)
2.之后在执行 task 的时候增加下面的参数
```groovy
./gradlew app:mytask -Dorg.gradle.debug=true
```
此时可以看到 gradle 在等待 debug 进程连接
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1579596969410-0ad288c6-03b7-4859-8578-e11be1773aa0.png)
3.之后在插件代码中打好断点，在 as 中点击 debug 按钮，就可以调试插件代码了
![](https://cdn.nlark.com/yuque/0/2020/webp/215777/1579577091992-b280a692-172a-4ede-94e8-a15afe7cc536.webp)
### 九、重点总结
主要要点如下图：
![](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWOWjqUlawJkTpg2nJIyInuPvN4icQvY50KEopwJxZJZnMmQn5FsVt5FA/640?wx_fmt=png&amp;from=appmsg)
其中一定要掌握的如下：

1. gradle dsl 查询地址 [docs.gradle.org/current/dsl…](https://docs.gradle.org/current/dsl/index.html)
2. android gradle plugin dsl 查询地址 [google.github.io/android-gra…](http://google.github.io/android-gradle-dsl/current/)
3. gradle 构建生命周期和回调
