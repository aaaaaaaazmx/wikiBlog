---
title: "【Android 修炼手册】主要 Task 分析-Gradle 篇"
published: 2020-07-29
description: "【Android 修炼手册】主要 Task 分析-Gradle 篇"
tags: ["Gradle","构建工具"]
category: "gradle"
draft: false
slug: "gradle-006"
...
大家可以直接 clone [EasyGradle](https://github.com/5A59/android-training/tree/master/gradle/EasyGradle) 项目，把 app/build.gradle 里的 implementation 'com.android.tools.build:gradle:3.0.1' 注释打开就可以了。
在 [Gradle的基本使用](https://juejin.im/post/5cd441f851882554b86d088b) 和 [Android Gradle Plugin 主要流程分析](https://juejin.im/post/5cd640526fb9a03232199a05) 里，我们知道了 gradle 中 task 的重要性，以及 android gradle plugin 的主要流程，这一篇就来分析一下 android gradle plugin 中一些重要的 task 是怎么执行的。
### 一、Android 打包流程
在介绍 Android Gradle Plugin Task 之前，我们先看看一个 apk 的构建流程，先放一张官方流程图：
![image.png](https://cdn.nlark.com/yuque/0/2020/png/215777/1580564222533-6da0952b-e375-4573-bf9f-123f05fcb560.png)
官方介绍的流程如下：
...
**transformClassesWithDexBuilderForDebug   (**class 打包dex**)**
...
GenerateBuildConfig  
##### 4.1.2 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWm5ibibGKQSPUibdckPrG5yH5WtDQfDibicnUBLPc5moqZD5EYRvZvsESbNw/640?wx_fmt=png&amp;from=appmsg)
##### 4.1.3 代码调用链路
```groovy
...
MergeResources
##### 4.2.2 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWESkXZeSnt5xl2nkP7UVD12CGXzib6ibHjYBydsWpHz5CAgaMZkwWBMfw/640?wx_fmt=png&amp;from=appmsg)
##### 4.2.3 调用链路
```groovy
...
##### 4.3.2 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWrGsnkPn7H8wbywV140NWnSP4Wmm0uiaSibyuUZwhhxSBxrW2blsAtl9A/640?wx_fmt=png&amp;from=appmsg)
##### 4.3.3 调用链路
```groovy
...
关于 aapt2 的 compile 和 link 参数，可以在 [developer.android.com/studio/comm…](https://developer.android.com/studio/command-line/aapt2#aapt2_changes) 这里看
#### 4.4 processDebugManifest
合并manifest文件
##### 4.4.1 实现类
MergeManifests
##### 4.4.2 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVW0X72HfeOuq5TvCAibDK5CaKyicxWiaT1oykcoc1mZ4kPwZ4NuXhzZpwtA/640?wx_fmt=png&amp;from=appmsg)
##### 4.4.3 调用链路
```groovy
...
8. 保存 manifest 到 build/intermediates/manifest/fullxxx/AndroidManifest.xml 这就生成了最终的 Manifest 文件
#### 4.5 transformClassesWithDexBuilderForDebug
class 打包 dex
##### 4.5.1 实现类
DexArchiveBuilderTransform
##### 4.5.2 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWKoluWjUrELye6bVPq1gDxMlcplZzAv9o39XAnrdXRN3gykc9MfudRg/640?wx_fmt=png&amp;from=appmsg)
##### 4.5.3 调用链路
```groovy
...
```
##### 4.5.4 主要代码分析
在 DexArchiveBuilderTransform 中，对 class 的处理分为两种方式，

1. 一种是对 目录下的 class 进行处理
2. 一种是对 .jar 里的 class 进行处理
...
打包三方库的 dex，在 dex 增量的时候就不需要再 merge 了，节省时间
##### 4.6.1 实现类
ExternalLibsMergerTransform
##### 4.6.2 整体实现图
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWwN0dicXR4ic43Ir0Av18pD57QVB6XPshwCFfNLrOLrdKC0vCt6sAxtAA/640?wx_fmt=png&amp;from=appmsg)
##### 4.6.3 调用链路
...
DexMergerTransform
##### 4.7.2 整体实现图
![](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWUiajOI6HsqE7ibdDia1lGjNu6icib5QGLK0luribOa3Qcagc42iaGYDGLsIMQ/640?wx_fmt=png&amp;from=appmsg)
##### 4.7.3 调用链路
和上一步类似
```
// dx 
