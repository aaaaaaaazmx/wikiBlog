---
title: "View机制的设计与实现：事件分发"
published: 2025-12-21
description: "View机制的设计与实现：事件分发"
tags: ["自定义View","Android","UI"]
category: "自定义View专栏"
draft: false
slug: "custom-view-014"
---

[https://juejin.cn/user/1503787635450584](https://juejin.cn/user/1503787635450584)
## 概述
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWbXsQPo7eXzd3zBj9J11WUzrR1iaczrXJQy5MZIvKwpM9TKgZGMqV36A/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://cdn.nlark.com/yuque/0/2024/png/215777/1711963705733-cef85b8b-172e-481c-8e3a-f1551989b471.png)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWEmkKyuzLgnGTEibHfhbnnicTib4a3NAsibkoj0qxxGqqksKHkpBzAq9ZUw/640?wx_fmt=png&amp;from=appmsg)
...
![image.png](https://mmbiz.qpic.cn/mmbiz_png/LrDMVD5bqLWkpfu3WMQgPVqETicpibhdVWWthuogSIFqHVZma0ZQfJHBPFSlZKN1u28ZqsbzVec4I1qClFTRf3WA/640?wx_fmt=png&amp;from=appmsg)
...
这个顶层的`View`其实就是`DecorView`（参见上文 _建立通信-ViewRootImpl的创建_ 小节），读者知道，`DecorView`实际上就是`Activity`中`Window`的根布局，它是一个`FrameLayout`。
...
上文中我们同样提到了，在`ViewGroup`的事件分发过程中，其本身的`dispatchTouchEvent(event)`和`super.dispatchTouchEvent(event)`完全是两个完全不同的函数，前者履行的是`ViewGroup`的职责，负责将事件分发给子`View`；后者履行的是`View`的职责，负责处理决定事件是否被消费（参见 _应用整体的事件分发-DecorView的双重职责_ 小节）。
...
![image.png](https://cdn.nlark.com/yuque/0/2024/png/215777/1711963869591-08345554-05b7-4d31-8a0a-ddca03db89ad.png)
